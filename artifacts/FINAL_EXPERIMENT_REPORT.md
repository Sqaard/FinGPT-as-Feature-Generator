# Final Experiment Report: Text Feature Groups for PPO
...
The primary completed ablation is the R6c frozen out-of-sample evaluation on
`fold_2021`. Older `custom_custom` PPO results are included as supporting
evidence, but they are not directly comparable to R6c because the policy
architecture, text schema, and execution mechanics differ.

## Main R6c Ablation Results

| Strategy | Return | Sharpe | Max drawdown | Return delta vs. baseline | Sharpe delta vs. baseline |
| --- | ---: | ---: | ---: | ---: | ---: |
| R6c baseline | -1.92% | -0.1015 | -11.78% | 0.00% | 0.0000 |
| R6c + all raw text10 | -1.61% | -0.0640 | -12.04% | +0.31% | +0.0374 |
...
All R6c text groups improved return and Sharpe relative to the no-text R6c
...
return benefit and larger drawdown make this group the least convincing R6c
...
| R6c strategy | Mean L1 turnover | Change vs. baseline | Max drawdown |
| --- | ---: | ---: | ---: |
...
repeat the same information many times; routing it only to the R6c root
...
Every selected R6c subset except sentiment had a better return than all
...
### 7. R6c is mechanically better suited to weak text signals

R6c uses normalized observations, constrained portfolio-weight actions,
...
For the full R6c text10 model, mean walk-forward validation return and Sharpe
...
2. Text can provide a modest relative improvement in R6c.
...
The R6c findings come from one frozen OOS fold and apparently one training
...
| R6c benefit is architectural | Run the same normalized compact features through flat and R6c policies with matched seeds | R6c retains the gain while flat PPO remains unstable |
