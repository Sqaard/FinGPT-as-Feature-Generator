#!/usr/bin/env python
"""Run task.md text-feature group ablations on the CHRL/R6c Stage0.1 agent.

The current CHRL model is stored in the original R6c launch package. It uses
the DeepSeek compact text10 schema, so the task groups are mapped onto those
PPO-facing columns. Each ablation keeps the same CHRL market state and sets
inactive text columns to zero in a group-specific raw panel. Training uses the
bundled Stage0.1 trainer, then this script evaluates the trained fold_2021
model on the frozen OOS window.
"""
...
OUT_ROOT = R6C_ROOT / "artifacts" / "chrl_text_feature_group_ablation"
REPO_OUT_ROOT = REPO_ROOT / "artifacts" / "chrl_text_feature_group_ablation"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=R6C_ROOT,
        help="Path to the CHRL runtime package. The current package directory still carries the historical R6c name.",
    )
...
def ensure_group_config(args: argparse.Namespace, group_key: str, group: dict[str, Any], panel_path: Path) -> Path:
    ...
    config["experiment"]["name"] = f"stage0_1_chrl_text_ablation_{group_key}"
    config["data"]["feature_set"] = f"stage0_1_weight_features_chrl_text_ablation_{group_key}"
    config["data"]["raw_features_csv"] = rel_to_project(panel_path, args.r6c_root)
    config["data"]["model_ready_csv"] = (
        f"artifacts/chrl_text_feature_group_ablation/features/{group_key}/"
    ...
    config["output"]["root_dir"] = "artifacts/chrl_text_feature_group_ablation/runs"
    ...
    config_path = args.out_root / "configs" / f"stage0_1_chrl_text_ablation_{group_key}.yaml"
...
def run_training(args: argparse.Namespace, group_key: str, config_path: Path) -> None:
    ...
    if all(path.exists() for path in completed) and not args.force_train:
        print(f"Using existing CHRL run for {group_key}")
        return
...
def collect_rows(args: argparse.Namespace, groups: dict[str, dict[str, Any]]) -> pd.DataFrame:
    ...
    if RAW_TEXT10_FROZEN_CSV.exists():
        ...
        if not raw.empty:
            ...
            rows.append(
                {
                    "strategy": "CHRL model + all raw text10",
    ...
    out["return_delta_vs_chrl_baseline"] = out["return"] - base_return
    out["sharpe_delta_vs_chrl_baseline"] = out["sharpe"] - base_sharpe

    def classify(row: pd.Series) -> str:
        if row["feature_group"] == "baseline":
            return "baseline"
        if row["return_delta_vs_chrl_baseline"] > 0 and row["sharpe_delta_vs_chrl_baseline"] > 0:
            return "useful"
        if row["return_delta_vs_chrl_baseline"] < -0.02 and row["sharpe_delta_vs_chrl_baseline"] < -0.15:
            return "harmful"
        return "noisy/mixed"
...
def write_report(args: argparse.Namespace, data_ready: dict[str, Any], groups: dict[str, dict[str, Any]], corr: pd.DataFrame, rows: pd.DataFrame) -> None:
    lines = [
        ...
        f"Raw CHRL/R6c panel: `{Path(data_ready['raw_panel']).relative_to(REPO_ROOT)}`.",
...
def write_outputs(args: argparse.Namespace, data_ready: dict[str, Any], groups: dict[str, dict[str, Any]], corr: pd.DataFrame, rows: pd.DataFrame) -> None:
    ...
    rows.to_csv(results_dir / "chrl_ablation_summary.csv", index=False)
    ...
    write_json(
        args.repo_out_root / "manifest.json",
        {
            ...
            "chrl_runtime_root": str(args.r6c_root),
            "variant": args.variant,
            "folds": args.folds,
            "groups": groups,
            "result_files": {
                "summary": str((results_dir / "chrl_ablation_summary.csv").relative_to(REPO_ROOT)),
